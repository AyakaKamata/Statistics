...
"""
univariate DLM
y_t = t(x_t) * beta_t + epsilon_t, x_t-N(0, sigma_x^2), epsilon_t-N(0, sigma_e^2)
beta_t = beta_{t-1} + delta_t(=drift), delta_t-N(0, sigma_d^2)
"""
...
# shape (n, p), sampled from N(0,0.1), cumulatively summed
...
class DLM(ForecastingModel):
    def model(self, zero_data, covariates):
        ...
        # drift(delta_t)のreparam用のスケールを対数正規分布からサンプリング
        ...
        # ドリフトの累積和を取ることで、ブラウン運動（ランダムウォーク）を生成
        # 列ごとに累積和（ここでは１列のみ）
...
class DLM2(ForecastingModel):
    def model(self, zero_data, covariates):
        ...
        drift_scale = pyro.sample(
            "drift_scale", dist.LogNormal(-10, 10).expand([feature_dim]).to_event(1)
        )
        with self.time_plate:
            with poutine.reparam(config={"drift": LocScaleReparam()}):
                drift = pyro.sample(
                    "drift",
                    dist.Normal(torch.zeros(covariates.size()), drift_scale).to_event(
                        1
                    ),
                )
        weight = drift.cumsum(-2)  # A Brownian motion.
        ...
        for tp, prior in zip(time_points, priors):
            ...
            pyro.sample(
                "weight_prior_{}".format(tp),  # サンプル名（各時間点ごとに異なる名前）
                # 分散が0.4以上だと大きすぎる
...
# posterior quantiles of latent variables
coef_p10, coef_p50, coef_p90 = quantile(
    posterior_samples2["weight"], (0.1, 0.5, 0.9)
).squeeze(-1)
